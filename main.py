...
def aproxima_neuronio(id_elemento,id_neuronio,cube,brain,tentativa=1):
    ...
    while i<6:
        while j<12:
            ...
            brain[i][id_neuronio][j]=(1-(alpha*0.02))*brain[i][id_neuronio][j]+(alpha)*0.02*(cube[i][id_elemento][j])
...
def dist_parcial_id(elemento,cube,brain,parcial,distancia):
    ...
    for brain_id in range(looping_start,(looping_end)):
        for j in range (0,5): #elemento 6 será usado para definir compra ou venda
            ...
            d=0
            try:
                if d0>=np.max(dist):#esse if torna as distancias maiores que a minima não confiaveis, mas acelera o processo
                    j=6
            except:
                pass
...
def dist_id_thread(elemento,cube,brain,distancia):
    ...
    p=mp.Pool(4)
    lista=[(elemento,cube,brain,0,distancia),(elemento,cube,brain,1,distancia),(elemento,cube,brain,2,distancia),(elemento,cube,brain,3,distancia)]
...
def aproxima_brain(cube,brain,tratados=[]):
    ...
    distancia_minima=0
    for i in range(0,k):
        to_be_aprox,distancia_minima=dist_id(i,cube,brain,distancia)#identifica qual neuronio esta mais proximo desse input#usar multiproc aqui #ToDo
...
def refaz_neuronios_pouco_usados(tratados,brain,cube,trying):
    alfa=tratados.min()
    if alfa<(tratados.max()*0.1):
        k = tratados.argmin()
...
def probabilidade_neuronio(analisaveis,brain):
    ...
    for i in range(0,30):
        ...
        if probabilidade[i]>0.15:#
            ...
            if probabilidade[i]<-0.15:
                brain[5][i][0]=0.5*brain[5][i][0]-0.5*2 #converge ffortemente para -2
            else:
...
if __name__ == '__main__':#Inicio
    ...
    tratados=np.zeros(30) #armazena quais neuronios foram aproximados e quantas vezes
    tupla=aproxima_brain(cube,brain,tratados)
    trying=1
    retrying=0
    print(f"elementos tratados: {tratados.sum()}")
    print(f'utilização do neuronio menos usado: {tratados.min()}')
    n_elementos=[]
    dist_media=[]
    prob = []
    ...
    while prob_acertos<0.65:
        while probabilidade_minima<0.15 and retrying<=5:
            if(argprobmin!=-1):
                tratados.fill(0)
                refaz_neuronios_pouco_usados(menos_provaveis, brain, cube, trying)
            ...
            for i in range(1,30):
                if probabilidade_minima>abs(prob[i]):
                    ...
                    menos_provaveis[i]=0 #funciona igual os elementos tratados, mas faz o neuronio com menor probabilidade ser o menor forcadamente
            retrying+=1
            total=0
        acertos=0
        n_elementos[:]=n_proximos_brain(brain,cube)
